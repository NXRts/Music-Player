...
export const useAudioPlayer = (currentSong, songs, options = {}) => {
  ...
  const setupAudioContext = useCallback(() => {
    ...
    if (audioRef1.current && !sourceRef1.current) {
      try {
        sourceRef1.current = audioContextRef.current.createMediaElementSource(
          audioRef1.current,
        );
        ...
        console.error("Source 1 setup error", e);
      }
    }
    ...
    if (audioRef2.current && !sourceRef2.current) {
      try {
        sourceRef2.current = audioContextRef.current.createMediaElementSource(
          audioRef2.current,
        );
        ...
        console.error("Source 2 setup error", e);
      }
    }
  ...
  const updateEqGain = useCallback(
    (index, value) => {
