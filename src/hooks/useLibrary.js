...
import { getSongMetadata, getAudioDuration, formatDuration } from '../utils/audioUtils';
...
export const useLibrary = () => {
    ...
    const updateSong = useCallback(async (songId, newData) => {
    ...
    const syncLocalFolder = useCallback(async () => {
        ...
        try {
            ...
            for (const fileObj of audioFiles) {
                ...
                // Check duplicates
                const isDuplicate = existingTitles.has(title) || existingTitles.has(`${title}-${artist}`);
            ...
            return { added: newSongs.length, total: audioFiles.length, folderName: folderHandle.name };
