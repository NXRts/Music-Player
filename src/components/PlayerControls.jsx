  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (
        ...
        !sleepMenuRef.current.contains(event.target)
      ) {
        ...
        {currentSong ? (
          <>
            <div className="relative group flex-shrink-0">
              {currentSong.cover &&
              !currentSong.cover.includes("placehold.co") ? (
                <img
            ...
            <div className="overflow-hidden min-w-0">
              <div className="font-bold hover:underline cursor-pointer truncate text-sm md:text-base">
                {currentSong.title}
              </div>
              <div className="text-xs text-text-secondary hover:underline cursor-pointer truncate">
                {currentSong.artist}
              </div>
            </div>
            <button
      ...
      <div className="flex flex-col items-center flex-1 md:w-1/2 max-w-[600px] gap-1">
        <div className="flex items-center gap-4 sm:gap-6 mb-1 md:mb-2">
          ...
          >
            <Repeat size={20} />
            {repeatMode === 2 && (
              <span className="absolute -top-1 -right-1 text-[8px] bg-accent text-black rounded-full w-3 h-3 flex items-center justify-center font-bold">
                1
              </span>
            )}
          </button>
          ...
          <div className="md:hidden relative" ref={mobileMenuRef}>
            ...
            {showMobileMenu && (
              <div className="absolute bottom-full mb-4 right-0 bg-bg-highlight border border-white/10 rounded-2xl shadow-2xl py-3 w-56 z-[200] animate-in slide-in-from-bottom-5 duration-300">
                <div className="px-4 py-2 text-xs font-bold text-text-secondary uppercase tracking-widest border-b border-white/5 mb-2">
                  Options
                </div>
                <button
                  onClick={() => {
                  ...
                  className={`w-full flex items-center gap-4 px-4 py-3 hover:bg-white/5 transition-colors ${currentSong && currentSong.isLiked ? "text-accent" : "text-text-primary"}`}
                >
                  <Heart
                    size={20}
                    fill={
                      currentSong && currentSong.isLiked
                        ? "currentColor"
                        : "none"
                    }
                  />{" "}
                  <span className="text-sm font-medium">Like Song</span>
                </button>
                <button
                  onClick={() => {
                    onToggleShuffle();
                  ...
                  className={`w-full flex items-center gap-4 px-4 py-3 hover:bg-white/5 transition-colors ${isShuffle ? "text-accent" : "text-text-primary"}`}
                >
                  <Shuffle size={20} />{" "}
                ...
                <button
                  ...
                  className={`w-full flex items-center gap-4 px-4 py-3 hover:bg-white/5 transition-colors ${repeatMode > 0 ? "text-accent" : "text-text-primary"}`}
                >
                  <Repeat size={20} />{" "}
                  <span className="text-sm font-medium">
                    Repeat {repeatMode === 2 ? "(One)" : ""}
                  </span>
                </button>
                <button
                  onClick={() => {
                  ...
                  className={`w-full flex items-center gap-4 px-4 py-3 hover:bg-white/5 transition-colors ${isLyricsOpen ? "text-accent" : "text-text-primary"}`}
                >
                  <Mic2 size={20} />{" "}
                ...
                <button
                  ...
                  className="w-full flex items-center gap-4 px-4 py-3 hover:bg-white/5 transition-colors text-text-primary"
                >
                  <Sliders size={20} />{" "}
                ...
                <button
                  ...
                  className={`w-full flex items-center gap-4 px-4 py-3 hover:bg-white/5 transition-colors ${isSleepTimerActive ? "text-accent" : "text-text-primary"}`}
                >
                  <Moon size={20} />{" "}
                ...
                >
                  <ListMusic size={20} />{" "}
                  <span className="text-sm font-medium">Open Queue</span>
                </button>
              </div>
            )}
          </div>
      ...
      <div className="hidden md:flex items-center gap-2 w-1/4 justify-end min-w-[200px]">
        ...
        <div className="relative" ref={sleepMenuRef}>
          ...
          >
            <Moon
              size={20}
              fill={isSleepTimerActive ? "currentColor" : "none"}
            />
          </button>
          {showSleepMenu && (
            <div className="absolute bottom-full mb-3 left-1/2 -translate-x-1/2 bg-bg-highlight border border-white/10 rounded-lg shadow-2xl py-2 w-40 z-50 animate-in fade-in slide-in-from-bottom-2 duration-200">
              <div className="text-xs font-bold text-text-secondary px-4 py-2 uppercase tracking-wider">
                Sleep Timer
              </div>
