...
const YourLibrary = ({
  ...
  const artists = useMemo(() => {
    ...
    songs.forEach((song) => {
      ...
      if (!groups[artist])
        groups[artist] = { name: artist, count: 0, cover: song.cover };
      groups[artist].count++;
    });
  ...
  if (selectedGroup) {
    const filteredSongs = songs.filter((song) => {
      if (selectedGroup.type === "artist")
        return (song.artist || "Unknown Artist") === selectedGroup.name;
      if (selectedGroup.type === "album")
        return (song.album || "Unknown Album") === selectedGroup.name;
      return false;
    });
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-text-secondary capitalize">
                {selectedGroup.type} • {filteredSongs.length} Songs
              </p>
            </div>
          </div>
        ...
        {activeTab === "playlists" &&
          (playlists.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-64 text-center text-text-secondary">
              ...
              <button
                onClick={onCreatePlaylist}
                className="mt-4 bg-white text-black font-bold px-8 py-3 rounded-full hover:scale-105 transition"
              >
                Create Playlist
              </button>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {artists.map((artist) => (
              <div
                key={artist.name}
                onClick={() =>
                  setSelectedGroup({ type: "artist", name: artist.name })
                }
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {albums.map((album) => (
              <div
                key={album.name}
                onClick={() =>
                  setSelectedGroup({ type: "album", name: album.name })
                }
