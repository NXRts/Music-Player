        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-8">
            ...
            <div className="flex items-center justify-between p-4 bg-bg-highlight/50 rounded-xl border border-border-subtle">
              <div>
                ...
                <p className="text-xs text-text-secondary">
                  Balance volume levels between different songs.
                </p>
              </div>
              <button
              ...
              >
                <div
                  className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-all ${isNormalizationEnabled ? "right-1" : "left-1"}`}
                ></div>
              </button>
            </div>
          </div>
          ...
          <div>
            ...
            <p className="text-text-secondary text-sm mb-4">
              ...
              {isSleepTimerActive && (
                <span className="ml-2 text-accent font-bold animate-pulse">
                  • Active
                </span>
              )}
            </p>
          ...
          {status && (
            <div
              className={`p-4 rounded-lg flex items-center gap-3 animate-fade-in ${status.type === "success" ? "bg-green-500/20 text-green-400" : "bg-red-500/20 text-red-400"}`}
            >
              {status.type === "success" ? (
                <CheckCircle size={20} />
              ) : (
