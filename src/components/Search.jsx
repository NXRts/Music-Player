          ...
          <div className="flex flex-col gap-2">
            {filteredSongs.map((song) => (
              <div
                key={song.id}
                onClick={() => onPlay(song)}
                className="flex items-center gap-4 p-3 bg-bg-highlight bg-opacity-40 hover:bg-opacity-100 rounded-md cursor-pointer transition group"
              >
                <img
                  src={song.cover}
                  alt={song.title}
                ...
                <div className="ml-auto opacity-0 group-hover:opacity-100 transition">
                  <span className="bg-accent text-black rounded-full w-8 h-8 flex items-center justify-center">
                    ▶
                  </span>
                </div>
              </div>
        ...
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {filteredCategories.map((genre, idx) => (
              <div
                key={idx}
                className={`h-32 rounded-lg p-4 font-bold text-xl relative overflow-hidden cursor-pointer transition hover:scale-[1.02]`}
