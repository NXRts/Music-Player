...
function App() {
  ...
  const [accentColor, setAccentColor] = useState(
    localStorage.getItem("accentColor") || "#1db954",
  );
  ...
  const playSong = async (song, shouldPlay = true) => {
    ...
    if (song.type === "local" && song.fileHandle) {
      try {
        const startStatus = await song.fileHandle.queryPermission({
          mode: "read",
        });
        if (startStatus !== "granted") {
          const requestStatus = await song.fileHandle.requestPermission({
            mode: "read",
          });
    ...
    const audio =
      activePlayer.current === 1 ? audioRef1.current : audioRef2.current;
    if (audio) {
      audio.src = songSrc;
      if (shouldPlay) {
        setupAudioContext();
    ...
    // History
    setHistory((prev) =>
      [song, ...prev.filter((s) => s.id !== song.id)].slice(0, 50),
    );
  };
  ...
  useEffect(() => {
    ...
    const onEnded = () => {
      if (repeatMode === 2) {
        ...
        repeatMode === 0 &&
        songs.findIndex((s) => s.id === currentSong?.id) === songs.length - 1 &&
        queue.length === 0
      ) {
        // End of list
        playerPause();
      ...
      <div className="flex-1 flex flex-col relative w-full">
        ...
        <main className="flex-1 flex overflow-hidden relative">
          <div className="flex-1 overflow-y-auto p-4 md:p-6 bg-gradient-to-b from-bg-highlight to-bg-primary relative">
              ...
              <>
                ...
                <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
                  {playlists.slice(0, 6).map((playlist) => {
                    const firstSong = playlist.songIds[0]
                      ? songs.find((s) => s.id === playlist.songIds[0])
                      : null;
                    return (
                      <div
                        key={playlist.id}
                        className="flex items-center bg-bg-card hover:bg-bg-highlight transition rounded-md overflow-hidden cursor-pointer group relative shadow-md"
                        onClick={() => {
                      ...
                      >
                        {firstSong?.cover &&
                        !firstSong.cover.includes("placehold.co") ? (
                          <img
                            src={firstSong.cover}
                            alt={playlist.name}
                            className="w-20 h-20 min-w-[5rem] object-cover shadow-lg"
                          />
                        ) : (
                        ...
                        <div className="px-4 flex-1 font-bold truncate text-sm md:text-base">
                          {playlist.name}
                        </div>
            ...
            {currentView === "search" && (
              <Search songs={songs} onPlay={handleSongSelect} />
            )}
              ...
              <div className="flex flex-col h-full">
                ...
                <SongList
                  songs={songs.filter((s) =>
                    activePlaylist.songIds.includes(s.id),
                  )}
                  currentSong={currentSong}
                  onSelect={(s) =>
                    handleSongSelect(
                      s,
                      songs.filter((x) =>
                        activePlaylist.songIds.includes(x.id),
                      ),
                    )
                  }
                  isPlaying={isPlaying}
        ...
        <div className="h-24 bg-bg-card border-t border-bg-highlight px-4 flex items-center justify-between z-50 relative">
          <PlayerControls
            ...
            onToggleLike={() =>
              updateSong(currentSong.id, { isLiked: !currentSong.isLiked })
            }
            onToggleQueue={() =>
              handleNavigate(currentView === "queue" ? "home" : "queue")
            }
                ...
                <div className="flex flex-col gap-2 max-h-60 overflow-y-auto mb-4">
                  {playlists.map((p) => (
                    <button
                      key={p.id}
                      onClick={() => {
                        addToPlaylist(p.id, songToAdd);
                        setShowPlaylistSelector(false);
                        showToast(`Added to ${p.name}`);
                      }}
                      className="p-3 hover:bg-bg-highlight rounded text-left"
                    >
                      {p.name}
                    </button>
                  ))}
